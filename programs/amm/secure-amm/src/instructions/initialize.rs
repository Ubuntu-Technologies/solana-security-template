//! Initialize - FIX: Strong PDA seeds with initializer + seed
...
pub struct Initialize<'info> {
    ...
    #[account(
        init,
        payer = initializer,
        // FIX: Includes seed for uniqueness - prevents collision/prediction
...
pub fn handler(ctx: Context<Initialize>, seed: u64, fee: u16) -> Result<()> {
    ...
    ctx.accounts.config.set_inner(Config {
        seed,
        authority: Some(ctx.accounts.initializer.key()),
