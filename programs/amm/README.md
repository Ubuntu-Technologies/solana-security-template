...
| Program | Purpose |
|---------|---------|
| **buggy-amm** | ❌ Multiple intentional vulnerabilities |
| **secure-amm** | ✅ Properly secured implementation |

## Vulnerabilities Demonstrated

### 1. Weak PDA Seeds
```rust
// ❌ buggy-amm: Globally predictable seeds
seeds = [b"config"]

// ✅ secure-amm: Unique per creator + seed
seeds = [b"amm", authority.key().as_ref(), &seed.to_le_bytes()]
```

### 2. Unchecked Arithmetic Overflow
```rust
// ❌ buggy-amm: Raw multiplication overflows
let product = amount_x * amount_y;

// ✅ secure-amm: u128 intermediates + checked math
...
```rust
// ❌ buggy-amm: Deposits without verifying vault ownership
pub vault_x: Account<'info, TokenAccount>,

// ✅ secure-amm: Vault authority must match config
...
3. **Verify all account relationships** - Don't trust caller-provided accounts
